...
train_df = pd.read_csv(
    "/lustre07/scratch/rayen/Oysters/datasets/train_df_nt_after_norm.csv"
)
train_df = train_df[
    ~train_df["ID"].isin(
        [
...
X2, y2, min_count2, ids2, nbr_segments2 = sequence_df(
    train_df2, segment_hours, "Sequence"
)
...
for group in pairs:
    ...
    for idx, img in zip(train_ids, X_train):
        img.resize((512, 512), Image.ANTIALIAS).save(
            os.path.join(folder_name, f"{idx}.png")
        )
...
segment_lengths = (
    all_results_df.groupby("segment_ids").size().reset_index(name="segment_length")
)
correct_predictions_count_by_segment = pd.merge(
    correct_predictions_count_by_segment, segment_lengths, on="segment_ids", how="left"
)

correct_predictions_count_by_segment["ID_acc"] = (
    correct_predictions_count_by_segment["correct_segments_count"]
    / correct_predictions_count_by_segment["segment_length"]
)
