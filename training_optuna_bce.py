...
def objective(trial):
    ...
    model.features[-1].fc2 = nn.Linear(
        in_features=1000, out_features=param["layer_size"], bias=True
    )
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=param["batch_size"], num_workers=4, shuffle=True
    )
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=param["batch_size"], num_workers=4, shuffle=True
    )
    ...
    model, best_acc, best_epoch = train_model(
        model, train_loader, test_loader, criterion, optimizer, device, param["epochs"]
    )
