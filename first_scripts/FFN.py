...
logging.basicConfig(
    filename=get_log_file_name("train", "log"), encoding="utf-8", level=logging.DEBUG
)
...
for fold, (train_indices, val_indices) in enumerate(kfold.split(dataset)):
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in train_dataloader:
            ...
            # Compute loss
            loss = criterion(
                outputs.view(len(targets), 1), targets.view(len(targets), 1).float()
            )
        ...
        with torch.no_grad():
            ...
            for val_inputs, val_targets in val_dataloader:
                ...
                val_loss += criterion(
                    val_outputs.view(len(val_targets), 1),
                    val_targets.view(len(val_targets), 1).float(),
                ).item()
