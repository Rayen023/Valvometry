...
for group in pairs:
    ...
    Seq_images = [
        resize(seq, (512, 512), anti_aliasing=False, preserve_range=True)
        for seq in X_train
    ]

    for idx, img in zip(train_ids, Seq_images):
        matplotlib.image.imsave(
            os.path.join(folder_name, f"{idx}.png"), img, cmap="gray"
        )
    ...
    Train_images = [
        resize(seq, (514, 563), anti_aliasing=False, preserve_range=True)
        for seq in X_train
    ]
    Test_images = [
        resize(seq, (514, 563), anti_aliasing=False, preserve_range=True)
