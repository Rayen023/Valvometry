...
def objective(trial, ids):
    ...
    for group in pairs:
        ...
        original_train_dataset = ImageDataset(
            X_train, y_train, train_ids, transform=test_transform
        )
        train_dataset = ImageDataset(
            X_train, y_train, train_ids, transform=train_transform
        )
        ...
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=batch_size, num_workers=16, shuffle=False
        )
        ...
        model.features[-1].fc2 = nn.Linear(
            in_features=1000, out_features=573, bias=True
        )
    ...
    acc = merged_df["pair_best_acc_mean"].mean()

    print(all_results_df)
    print(merged_df)

    folder_name = f"efficientnet_b0 lr_{lr}_epochs_{num_epochs}_batch_{batch_size}_img_{img_size}_segments_{segment_hours}h"
