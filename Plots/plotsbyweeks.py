...
# Read the CSV file
df = pd.read_csv(
    "/gpfs/scratch/rayen/Oysters/datasets/train_df2_after_norm_mem_reduce.csv"
)
...
for name, group in grouped:
    week = 0
    for i in range(
        0, len(group), int(len(group) * 7 / 35)
    ):  # 35 jours, 860 hours #30966466 ms
        ...
        plt.plot(
            group.index[i : i + int(len(group) * 7 / 35)],
            group["Sequence"][i : i + int(len(group) * 7 / 35)],
        )
